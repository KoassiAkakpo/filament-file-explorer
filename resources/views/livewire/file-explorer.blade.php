        ...
        >
            <div class="fe-finder w-full overflow-hidden rounded-xl border border-zinc-200/90 bg-zinc-50/80 shadow-sm dark:border-zinc-700 dark:bg-zinc-900/40" dir="ltr" lang="{{ $locale }}" translate="no">
                <div class="fe-body flex min-h-[560px]">
                    ...
                    <div class="fe-main flex min-w-0 flex-1 flex-col">
                {{-- Toolbar --}}
                ...
                <div class="fe-browser relative overflow-x-hidden dark:bg-zinc-800/50" x-bind:class="dropingFile ? 'fe-browser--drop' : ''">
                    @if ($showTrash)
                        ...
                        <div class="fe-trash-list flex-1 overflow-y-auto p-2">
                            @if ($trashRows === [])
                    ...
                    <div
                        ...
                        @class([
                            'relative flex-1 select-none overflow-y-auto p-2 pb-8',
                    ...
                    >
                        ...
                        @if ($viewMode === 'columns' && $panes !== [])
                            {{-- One pane per level of the path. Only the last
                                 pane carries data-fe-type / data-id, so the
                                 selection, the keyboard and the marquee keep
                                 operating on the folder being browsed exactly as
                            ...
                            @foreach ($panes as $pane)
                                ...
                                >
                                    ...
                                    @if ($pane['folders']->isEmpty() && $pane['files']->isEmpty())
                                        <p class="fe-column__empty">{{ __('filament-file-explorer::file-explorer.listing.empty') }}</p>
                                    @endif

                                    @if ($pane['isLast'] && $pane['hasMore'])
                                        <button type="button" class="fe-column__more" wire:click="loadMore" wire:loading.attr="disabled">
